...
const TOL = 4;          // min overlap on BOTH axes to count
const MIN_AREA = 300;   // min overlapping area (px^2) to count
...
async function analyze(page) {
  return page.evaluate((cfg) => {
    ...
    const CHROME = /\b(masthead|pagination|pager|dots|progress|timer|tm-btn|copy-btn|page-num)\b/;
    ...
    const hasBox = (el) => {
      const s = getComputedStyle(el);
      // Flip-card faces are intentionally stacked (one rotated 180°); their 2D boxes
      // "overlap" but it's a 3D flip, not a layout bug. backface-visibility:hidden is the tell.
      if (s.backfaceVisibility === "hidden" || el.closest('[style*="rotateY"], .flip, .flip-card, .face')) return false;
    ...
    const boxes = Array.from(content.querySelectorAll("*")).filter((el) => {
      ...
      if (!visible(el) || !hasBox(el)) return false;
      const r = el.getBoundingClientRect();
...
async function testDeck(file) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    await sleep(60);
    const steps = await page.evaluate((n) => window.deck.stepCount(n), i);
    for (let k = 0; k < steps; k++) { await page.keyboard.press("ArrowRight"); await sleep(40); }
    await sleep(80);
