# ABOUTME: Generates a consolidated markdown report from individual JSON research results for Agentic AI.
...
def main():
    ...
    lines = []
    lines.append(
        "# Agentic AI 2024-2026 — Research Report\n"
    )
    lines.append(
        "> Comprehensive mapping of the agentic AI landscape — frameworks, protocols, architectural "
        "patterns, products, market data, and emerging standards. Emphasis on build-vs-buy decisions "
        "for AI entrepreneurs, with practical deployment considerations.\n"
    )
    ...
    lines.append(
        "| # | Item | Type | Creator | Stars | Readiness | Autonomy | License |"
    )
    lines.append("|---|------|------|---------|-------|-----------|----------|---------|")
    ...
    for i, item in enumerate(items, 1):
        ...
        creator_val = get_toc_field_value(item["data"], "creator")
        stars_val = get_toc_field_value(item["data"], "github_stars")
        readiness_val = get_toc_field_value(item["data"], "production_readiness_level")
        autonomy_val = get_toc_field_value(item["data"], "autonomy_level")
        license_val = get_toc_field_value(item["data"], "license")
        lines.append(
            f"| {i} | {name_link} | {type_val} | {creator_val} | {stars_val} | {readiness_val} | {autonomy_val} | {license_val} |"
        )
