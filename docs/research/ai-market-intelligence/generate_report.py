# ABOUTME: Generates a markdown report from AI Market Intelligence research JSON files.
...
CATEGORY_MAPPING = {
    "metric_identity": ["metric_identity", "Metric Identity"],
    "metric_value": ["metric_value", "Metric Value"],
    "trend": ["trend", "Trend"],
...
CATEGORY_LABELS = {
    "report": "Major Reports & Indices",
    "market_metric": "Global Market Metrics",
    "cost_metric": "Cost & Efficiency Metrics",
    "adoption_metric": "Adoption & Growth Metrics",
    "infrastructure": "Infrastructure & Strategic",
    "regional": "Regional & European Focus",
    "labor": "Labor & Talent",
    "risk": "Risk & Governance",
    "competition": "Competition & Ecosystem",
...
def load_json(path):
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    # Some agents produce a list of metrics; take first item
...
def get_field_value(data, field_name):
    ...
    for key, val in data.items():
        ...
        # Skip non-dict values (e.g., uncertain list)
    return None
...
def generate_report():
    ...
    for cat in categories:
        ...
        lines.append(f"\n### {cat_label}\n")
        lines.append(
            "| # | Metric | Value | Scope | Trend | Quality |"
        )
        lines.append(
            "|---|--------|-------|-------|-------|---------|"
        )
        ...
        for item in category_items[cat]:
            ...
            if matches:
                ...
                val = truncate_for_toc(get_field_value(data, "value"), 60)
                scope = get_field_value(data, "geographic_scope") or "—"
                trend = get_field_value(data, "trend_direction") or "—"
                quality = get_field_value(data, "data_quality") or "—"
            else:
                val = scope = trend = quality = "—"

            lines.append(
                f"| {item_num} | [{name}](#{slug}) | {val} | {scope} | {trend} | {quality} |"
            )
