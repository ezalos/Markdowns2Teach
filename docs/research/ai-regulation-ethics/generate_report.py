# ABOUTME: Generates a consolidated markdown report from individual JSON research results.
...
TOC_SUMMARY_FIELDS = ["type", "jurisdiction", "status", "session_mapping"]

# Category mapping: fields.yaml key -> possible JSON keys
CATEGORY_MAPPING = {
    "basic_information": ["basic_information", "Basic Information", "basic_info"],
    "scope_and_content": ["scope_and_content", "Scope and Content", "scope_content"],
    ...
    "confidence": ["confidence", "Confidence", "data_confidence"],
}
...
def main():
    ...
    lines = []
    lines.append(
        "# AI Regulation, Ethics & Governance 2024-2026 — Research Report\n"
    )
    lines.append(
        "> Comprehensive mapping of the global AI regulatory landscape — legislation, standards, "
        "ethics frameworks, enforcement mechanisms, and emerging issue areas.\n"
    )
    ...
    lines.append(
        "| # | Item | Type | Jurisdiction | Status | Session |"
    )
    lines.append("|---|------|------|-------------|--------|---------|")
    ...
    for i, item in enumerate(items, 1):
        ...
        jurisdiction_val = get_toc_field_value(item["data"], "jurisdiction")
        status_val = get_toc_field_value(item["data"], "status")
        session_val = get_toc_field_value(item["data"], "session_mapping")
        # Shorten session_mapping for table
        if len(session_val) > 20:
            # Extract just the session number(s)
            session_match = re.findall(r"Session\s*(\d)", session_val)
            if session_match:
                session_val = ", ".join(f"S{n}" for n in session_match)
        lines.append(
            f"| {i} | {name_link} | {type_val} | {jurisdiction_val} | {status_val} | {session_val} |"
        )
