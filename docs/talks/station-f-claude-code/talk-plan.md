...
| 15 | **Reach shared understanding before you build** | TEACH | The Grill Me pattern: a two-line skill that interviews you relentlessly, 40-100 questions, until you and the agent share a design. Why it beats plan mode: plan mode is *"extremely eager to create an asset"* instead of reaching understanding. **Live-demo candidate.** |
...
| # | Slide | Pace | Content |
|---|---|---|---|
| 26 | **Stop prompting. Build loops.** | TEACH | WHY+WHAT. The shift from "write this for me" to "keep working toward this". A loop that re-prompts itself is what makes long-running and parallel work possible. |
...
1. **The live demo** — slide 15 (Grill Me) is the natural one, ~3 min, needs one rehearsal. In or out?
2. **Boris Cherny's name on the loops slide** — he has no primary linkable source for it. Recommendation: drop the name, keep the claim on Steinberger + Anthropic.
3. **Chapter 5 ordering** — memory before distribution, or distribution first? Current order builds from what they own to what they can reach.
