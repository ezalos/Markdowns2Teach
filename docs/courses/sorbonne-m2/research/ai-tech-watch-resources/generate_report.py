# ABOUTME: Generates report.md from individual JSON research files for AI Tech Watch & Learning Resources.
# ABOUTME: Reads fields.yaml for structure, processes 35 JSONs from results/, outputs a categorized markdown report.
...
CATEGORY_MAPPING = {
    ...
    "quality_and_accessibility": ["quality_and_accessibility"],
    "entrepreneurial_relevance": ["entrepreneurial_relevance"],
    "currency_and_ecosystem": ["currency_and_ecosystem"],
...
def format_value(val):
    ...
    val_str = str(val)
    if len(val_str) > 200:
        # Use blockquote for long text
        return val_str
    return val_str


def format_toc_value(val):
...
def main():
    ...
    # Build explicit mapping from source filename to outline item
    # This is the most reliable approach since filenames were generated from outline names
    def normalize(s):
    ...
    for res in resources:
        name = res.get("resource_name", res["_source_file"])
    ...
    # Build report
    lines = []
    lines.append("# AI Tech Watch & Learning Resources 2024-2026")
    lines.append("")
    lines.append("> Curated resource guide for M2 Entrepreneurship students (Sorbonne).")
    lines.append("> 35 resources across 10 categories. Business-oriented, accessible, current as of February 2026.")
    lines.append("> French/European resources prioritized.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    for cat_name in outline_categories:
        ...
        lines.append("")
        lines.append("| # | Resource | Type | Lang | Tier | Access | Sessions |")
        lines.append("|---|----------|------|------|------|--------|----------|")
        ...
        for res in cat_resources:
            ...
            anchor = slugify(name)
            r_type = format_toc_value(get_field_value(res, "resource_type"))
            lang = format_toc_value(get_field_value(res, "language"))
            tier = format_toc_value(get_field_value(res, "recommendation_tier"))
            access = format_toc_value(get_field_value(res, "free_or_paid"))
            sessions = format_toc_value(get_field_value(res, "session_mapping"))
            # Shorten access for table
            if len(access) > 30:
                access = access[:27] + "..."
            if len(sessions) > 30:
                sessions = sessions[:27] + "..."
            lines.append(
                f"| {global_idx} | [{name}](#{anchor}) | {r_type} | {lang} | {tier} | {access} | {sessions} |"
            )
    ...
    if unmatched:
        lines.append("## Uncategorized Resources")
        lines.append("")
        for res in unmatched:
            name = res.get("resource_name", res.get("_source_file", "Unknown"))
            lines.append(f"### {name}")
            lines.append("")
